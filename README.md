...
3. **抓不到点赞/阅读量等互动数据**：
   受限于只能抓取网页文章列表页设计，本程序**只能完美抓取“所有文章元数据列表”和“文章内部的纯文本（Content）”**。至于一篇文章的阅读数、点赞数、评论数等，因保护极严只能通过截获微信手机客户端的通讯下发参数实现，此类纯外部浏览器爬虫是做不到的。
4. **频率限制**：
   单个账号请求抓取间隔默认 **3 秒**，若爬取超过几千篇的庞大公众号，请在 `config.json` 中适当调高 `delay_seconds`。长期高频狂刷会导致 IP 被微信官方拦截 24 小时。
5. 仅用于个人学习研究，请尊重著作权。
